...
data class ProcessedElemMessage(val elementSimpleName: String) {
  ...
  companion object {
    ...
    fun parseAllIn(s: String): List<ProcessedElemMessage> {
      ...
      return pattern
        .findAll(s)
        .map { match -> ProcessedElemMessage(match.destructured.component1()) }
        .toList()
    }
  }
}
...
class KotlinTestProcessor : AbstractProcessor() {
  ...
  override fun process(annotations: Set<TypeElement>, roundEnv: RoundEnvironment): Boolean {
    ...
    if (annotations.isEmpty()) {
      FileSpec.builder(GENERATED_PACKAGE, GENERATED_KOTLIN_CLASS_NAME + ".kt")
        .addType(TypeSpec.classBuilder(GENERATED_KOTLIN_CLASS_NAME).build())
        .build()
        .let { writeKotlinFile(it) }

      JavaFile.builder(
          GENERATED_PACKAGE,
          JavaTypeSpec.classBuilder(GENERATED_JAVA_CLASS_NAME).build(),
        )
        .build()
  ...
  private fun writeKotlinFile(
    ...
    packageName: String = fileSpec.packageName,
  ) {
