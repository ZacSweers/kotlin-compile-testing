...
class KotlinCompilationTests {
  ...
  fun `runs with only kotlin sources`() {
    val result =
      defaultCompilerConfig()
        .apply { sources = listOf(SourceFile.kotlin("kSource.kt", "class KSource")) }
        .compile()
  ...
  fun `runs with only java sources`() {
    val result =
      defaultCompilerConfig()
        .apply { sources = listOf(SourceFile.java("JSource.java", "class JSource {}")) }
        .compile()
  ...
  fun `runs with no sources`() {
    val result = defaultCompilerConfig().apply { sources = emptyList() }.compile()
  ...
  fun `runs with SourceFile from path`() {
    val sourceFile = temporaryFolder.newFile("KSource.kt").apply { writeText("class KSource") }

    val result =
      defaultCompilerConfig().apply { sources = listOf(SourceFile.fromPath(sourceFile)) }.compile()
  ...
  fun `runs with SourceFile from paths with filename conflicts`() {
    ...
    val sourceFileA =
      temporaryFolder.newFile("a/KSource.kt").apply { writeText("package a\n\nclass KSource") }

    temporaryFolder.newFolder("b")
    val sourceFileB =
      temporaryFolder.newFile("b/KSource.kt").apply { writeText("package b\n\nclass KSource") }

    val result =
      defaultCompilerConfig()
        .apply {
          sources = listOf(SourceFile.fromPath(sourceFileA), SourceFile.fromPath(sourceFileB))
        }
        .compile()
  ...
  fun `Kotlin can access JDK`() {
    ...
    val result = defaultCompilerConfig().apply { sources = listOf(source) }.compile()
  ...
  fun `Java can not access JDK`() {
    ...
    assertThat(result.messages)
      .contains("jdkHome is set to null, removing boot classpath from java compilation")
  }
  ...
  fun `Kotlin doesn't inherit classpath`() {
    ...
    assertThat(result.exitCode).isEqualTo(ExitCode.COMPILATION_ERROR)
    assertThat(result.messages)
      .containsIgnoringCase("Unresolved reference 'KotlinCompilationTests'")
  }
  ...
  fun `Kotlin can access Java class`() {
    ...
    val result = defaultCompilerConfig().apply { sources = listOf(kSource, jSource) }.compile()
  ...
  fun `Given only Java sources, Kotlin sources are generated and compiled`() {
    ...
    val clazz =
      result.classLoader.loadClass(
        KotlinTestProcessor.GENERATED_PACKAGE +
          "." +
  ...
  fun `Java can access generated Kotlin class`() {
    ...
    assertClassLoadable(
      result,
      "${KotlinTestProcessor.GENERATED_PACKAGE}.${KotlinTestProcessor.GENERATED_KOTLIN_CLASS_NAME}",
    )
  }
  ...
  fun `Java can access generated Java class`() {
    ...
    assertClassLoadable(
      result,
      "${KotlinTestProcessor.GENERATED_PACKAGE}.${KotlinTestProcessor.GENERATED_JAVA_CLASS_NAME}",
    )
  }
  ...
  fun `Generated Java source is among generated files list`() {
    ...
    assertThat(result.exitCode).isEqualTo(ExitCode.OK)
    assertThat(result.generatedFiles.map { it.name })
      .contains(KotlinTestProcessor.GENERATED_JAVA_CLASS_NAME + ".java")
  }
  ...
  fun `Generated Kotlin source is among generated files list`() {
    ...
    assertThat(result.exitCode).isEqualTo(ExitCode.OK)
    assertThat(result.generatedFiles.map { it.name })
      .contains(KotlinTestProcessor.GENERATED_KOTLIN_CLASS_NAME + ".kt")
  }
  ...
  fun `Custom plugin receives CLI argument`() {
    ...
    verify(cliProcessor, atLeastOnce())
      .processOption(
        argWhere<AbstractCliOption> { it.optionName == "test_option_name" },
        eq("test_value"),
        any(),
      )
    verify(cliProcessor, never())
      .processOption(
        argWhere<AbstractCliOption> { it.optionName == "test_option_name" },
        not(eq("test_value")),
        any(),
  ...
  fun `java compilation runs in process when no jdk is specified`() {
    ...
    assertThat(result.messages)
      .doesNotContain("jdkHome is set to null, removing boot classpath from java compilation")
  }
  ...
  fun `java compilation runs in a sub-process when jdk is specified`() {
    ...
    assertThat(logs).contains("compiling java in a sub-process because a jdkHome is specified")
    assertThat(logs)
      .doesNotContain("jdkHome is set to null, removing boot classpath from java compilation")
    fakeJdkHome.delete()
  }
    ...
    val result =
      defaultCompilerConfig()
        ...
        .apply {
          ...
          assertThat(outputDirectory.listFilesRecursively().map { it.name })
            .contains("KSource1.class")
        }
