...
class KotlinJsCompilationTests {
  ...
  fun `runs with no sources`() {
    val result = defaultJsCompilerConfig().apply { sources = emptyList() }.compile()
  ...
  fun `runs with SourceFile from path`() {
    ...
    val result =
      defaultJsCompilerConfig()
        .apply { sources = listOf(SourceFile.fromPath(sourceFile)) }
        .compile()
...
""",
    ...
    val result = defaultJsCompilerConfig().apply { sources = listOf(source) }.compile()
