...
internal class KctKaptCompilerPluginRegistrar(
  ...
  override fun ExtensionStorage.registerExtensions(configuration: CompilerConfiguration) {
    ...
    fun abortAnalysis() =
      FirAnalysisHandlerExtension.Companion.registerExtension(AbortAnalysisHandlerExtension())
    ...
    FirAnalysisHandlerExtension.Companion.registerExtension(
      kaptFirAnalysisCompletedHandlerExtension
    )
  }
  ...
  private fun KaptOptions.Builder.checkOptions(
    ...
    if (classesOutputDir == null) {
      if (configuration.get(JVMConfigurationKeys.OUTPUT_JAR) != null) {
        logger.error(
          "Kapt does not support specifying JAR file outputs. Please specify the classes output directory explicitly."
        )
        abortAnalysis()
        return false
      } else {
    ...
    if (sourcesOutputDir == null || classesOutputDir == null || stubsOutputDir == null) {
      if (mode != AptMode.WITH_COMPILATION) {
        ...
        logger.warn(
          "$nonExistentOptionName is not specified for $moduleName, skipping annotation processing"
        )
        abortAnalysis()
      }
      return false
