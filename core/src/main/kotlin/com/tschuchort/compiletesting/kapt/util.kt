...
fun KaptOptions.Builder.toPluginOptions(): List<String> {
  ...
  for (option in KaptCliOption.entries) {
    fun Any.pluginOption(value: String = this.toString()) {
      options +=
        listOf(
          "-P",
          "plugin:" +
            KaptCliOption.ANNOTATION_PROCESSING_COMPILER_PLUGIN_ID +
            ":" +
            option.optionName +
            "=" +
            value,
    ...
    when (option) {
      ...
      KaptCliOption.COMPILED_SOURCES_DIR ->
        compiledSources.joinToString(File.pathSeparator).pluginOption()
      ...
      KaptCliOption.ANNOTATION_PROCESSORS_OPTION ->
        processors.map(String::trim).filterNot(String::isEmpty).joinToString(",").pluginOption()
      ...
      KaptCliOption.USE_LIGHT_ANALYSIS_OPTION ->
        (KaptFlag.USE_LIGHT_ANALYSIS in flags).pluginOption()
      ...
      KaptCliOption.MAP_DIAGNOSTIC_LOCATIONS_OPTION ->
        (KaptFlag.MAP_DIAGNOSTIC_LOCATIONS in flags).pluginOption()
      ...
      KaptCliOption.INCLUDE_COMPILE_CLASSPATH ->
        (KaptFlag.INCLUDE_COMPILE_CLASSPATH in flags).pluginOption()
