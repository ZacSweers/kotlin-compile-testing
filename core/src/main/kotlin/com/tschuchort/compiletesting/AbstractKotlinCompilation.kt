...
/**
 * Base compilation class for sharing common compiler arguments and functionality. Should not be
 * used outside of this library as it is an implementation detail.
 */
...
abstract class AbstractKotlinCompilation<A : CommonCompilerArguments> internal constructor() {
  ...
  /**
   * Paths to directories or .jar files that contain classes to be made available in the compilation
   * (i.e. added to the classpath)
   */
  ...
  /**
   * Helpful information (if [verbose] = true) and the compiler system output will be written to
   * this stream
   */
  ...
  /**
   * Path to the kotlin-stdlib-common.jar If none is given, it will be searched for in the host
   * process' classpaths
   */
  var kotlinStdLibCommonJar: File? by default { HostEnvironment.kotlinStdLibCommonJar }
  ...
  protected val sourcesDir
    get() = workingDir.resolve("sources")
  ...
  protected fun commonArguments(args: A, configuration: (args: A) -> Unit): A {
    ...
    /**
     * It's not possible to pass dynamic [CommandLineProcessor] instances directly to the
     * [K2JSCompiler] because the compiler discovers them on the classpath through a service
     * locator, so we need to apply the same trick as with [CompilerPluginRegistrar]s: We put our
     * own static [CommandLineProcessor] on the classpath which in turn calls the user's dynamic
     * [CommandLineProcessor] instances.
     */
    ...
    /**
     * Our [MainCommandLineProcessor] only has access to the CLI options that belong to its own
     * plugin ID. So in order to be able to access CLI options that are meant for other
     * [CommandLineProcessor]s we wrap these CLI options, send them to our own plugin ID and later
     * unwrap them again to forward them to the correct [CommandLineProcessor].
     */
    args.pluginOptions =
      pluginOptions
        .map { (pluginId, optionName, optionValue) ->
  ...
  protected fun compileKotlin(
    ...
    val generatedSourceFiles =
      extraGeneratedSources
        ...
        .filter(File::hasSourceFileExtension)
        .map { it.absoluteFile }
     ...
     *
     * To avoid that the annotation processors are executed twice, the list is set to empty
     */
    ...
    val args =
      arguments.also { args ->
        args.freeArgs =
          finalSources.map(File::getAbsolutePath).distinct() +
            if (finalSources.none(File::hasKotlinFileExtension)) {
  ...
  protected fun searchSystemOutForKnownErrors(compilerSystemOut: String) {
    if (
      compilerSystemOut.contains("No enum constant com.sun.tools.javac.main.Option.BOOT_CLASS_PATH")
    ) {
      warn(
  ...
  protected val internalMessageStream =
    PrintStream(
      TeeOutputStream(
        object : OutputStream() {
          ...
          override fun write(b: ByteArray, off: Int, len: Int) =
            messageOutputStream.write(b, off, len)
  ...
  protected val diagnostics: List<DiagnosticMessage>
    get() = _diagnostics
  ...
  internal fun createMessageCollectorAccess(stepName: String): MessageCollector =
    createMessageCollector(stepName)
...
@ExperimentalCompilerApi
internal fun convertKotlinExitCode(code: ExitCode) =
  when (code) {
